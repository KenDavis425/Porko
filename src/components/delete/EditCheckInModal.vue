<template>
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content" v-if="editableCheckIn">
      <h2>Edit Check-In</h2>
      <form @submit.prevent="onSave">
        <div class="form-group">
          ...
          <div class="star-rating-input">
            <span v-for="star in 5" :key="star"
              ...
              @click="editableCheckIn.rating = star"
        ...
        <div class="form-group">
          <label>Photo</label>
          <div v-if="!photoDataUrl && !editableCheckIn.photoURL" class="photo-actions">
          ...
          <div v-else class="photo-preview">
            <img :src="photoDataUrl || editableCheckIn.photoURL" alt="Check-in photo preview" />
        ...
        <div class="form-group">
          ...
          <textarea id="comment" v-model="editableCheckIn.comment" rows="4"></textarea>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="tags-container">
            <button
              ...
              type="button"
              class="tag-btn"
              :class="{ selected: editableCheckIn.tags.includes(tag.name) }"
...
export default {
  ...
  emits: ['close', 'save'],
  setup(props, { emit }) {
    const editableCheckIn = ref(null);
    ...
    watch(() => props.checkin, (newVal) => {
      if (newVal) {
        editableCheckIn.value = JSON.parse(JSON.stringify(newVal));
        if (!editableCheckIn.value.tags) {
          editableCheckIn.value.tags = [];
        }
        photoDataUrl.value = null; // Reset photo preview on new checkin
    ...
    const getStarPath = (starIndex) => {
      const displayRating = hoverRating.value || (editableCheckIn.value ? editableCheckIn.value.rating : 0);
      return displayRating >= starIndex ? fullStarPath : emptyStarPath;
    };

    const toggleTag = (tagName) => {
      if (!editableCheckIn.value) return;
      const tags = editableCheckIn.value.tags;
    ...
    const onSave = () => {
      emit('save', {
        checkinData: editableCheckIn.value,
    ...
    const capturePhoto = () => {
      ...
      photoRemoved.value = false; // A new photo is captured, so it's not "removed"
      closeCamera();
    };

    const removePhoto = () => {
      photoDataUrl.value = null;
      if (editableCheckIn.value) {
        editableCheckIn.value.photoURL = null;
      }
